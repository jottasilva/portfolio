  ...
  return (
    <div className={css({ display: 'flex', minH: '100vh', bg: '#090909' })}>
      ...
      <div className={css({ w: '260px', bg: '#0d0d0d', borderRight: '1px solid rgba(255,255,255,0.03)', p: 6, display: 'flex', flexDir: 'column', gap: 8 })}>
        ...
        <nav className={css({ display: 'flex', flexDir: 'column', gap: 2, flex: 1 })}>
          {[
            ...
            <button
              ...
              className={css({
                w: 'full', textAlign: 'left', p: 3, rounded: '2px', fontSize: 'sm', fontFamily: 'headline',
        ...
        <button onClick={() => logout()} className={css({ w: 'full', p: 3, rounded: '2px', bg: 'rgba(255,0,0,0.05)', color: '#ff4444', border: '1px solid rgba(255,0,0,0.1)', fontSize: 'xs', cursor: 'pointer', _hover: { bg: 'rgba(255,0,0,0.1)' } })}>
          Sair do Sistema
        </button>
      </div>
        ...
        <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className={css({ display: 'flex', flexDir: 'column', gap: 8 })}>
              ...
              <div className={css({ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 5 })}>
                {[
                  ...
                  <div key={kpi.label} className={css({ bg: '#141414', p: 6, rounded: '2px', border: '1px solid rgba(255,255,255,0.04)', position: 'relative', overflow: 'hidden' })}>
              ...
              <div className={css({ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 6 })}>
                ...
                {/* Top Cidades */}
                <div className={css({ bg: '#141414', p: 6, rounded: '2px', border: '1px solid rgba(255,255,255,0.04)' })}>
                ...
                {/* Visitas Recentes */}
                <div className={css({ bg: '#141414', p: 6, rounded: '2px', border: '1px solid rgba(255,255,255,0.04)', overflow: 'hidden' })}>
                  <div className={css({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 5 })}>
                    ...
                    <input 
                      ...
                      className={css({ bg: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.05)', rounded: '2px', px: 3, py: 1.5, fontSize: 'xs', color: 'white', outline: 'none', _focus: { borderColor: 'primary' } })}
                    />
                  </div>
                ...
                <div className={css({ bg: '#141414', p: 6, rounded: '2px', border: '1px solid rgba(255,255,255,0.04)' })}>
                  ...
                  <div className={css({ display: 'flex', flexDir: 'column', gap: 3 })}>
                    {messages.slice(0, 3).map((m) => (
                      <div key={m.id} className={css({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 3, bg: 'rgba(255,255,255,0.02)', rounded: '2px', borderLeft: '2px solid token(colors.secondary)' })}>
                        <div>
          ...
          {activeTab === 'messages' && !loading && (
            <div className={css({ display: 'flex', flexDir: 'column', gap: 4 })}>
              {messages.map((m) => (
                <div key={m.id} className={css({ bg: '#121212', p: 5, rounded: '2px', borderLeft: '3px solid token(colors.secondary)' })}>
          ...
          {activeTab === 'chat' && !loading && (
            <div className={css({ display: 'flex', flexDir: 'column', gap: 4 })}>
              ...
              {chatError && (
                <div className={css({ bg: 'rgba(255,152,0,0.1)', p: 5, rounded: '2px', borderLeft: '3px solid #ff9800', mb: 2 })}>
              ...
              {chatMessages.map((m) => (
                <div key={m.id} className={css({ bg: '#121212', p: 5, rounded: '2px', borderLeft: '3px solid token(colors.primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' })}>
          ...
          {activeTab === 'about' && !loading && (
            <form onSubmit={handleSaveAbout} className={css({ bg: '#141414', p: 6, rounded: '2px', display: 'flex', flexDir: 'column', gap: 4, maxW: '3xl' })}>
              ...
              <div>
                ...
                <input required value={(about as any).name || ''} onChange={e=>setAbout({ ...about, name: e.target.value } as any)} className={css({ w: 'full', bg: '#1a1a1a', p: 3, rounded: '2px', border: 'none', color: 'white', mt: 1 })}/>
              </div>

              <div>
                <label className={css({ color: 'gray.400', fontSize: 'xs' })}>Cargo / Especialidade</label>
                <input required value={(about as any).role || ''} onChange={e=>setAbout({ ...about, role: e.target.value } as any)} className={css({ w: 'full', bg: '#1a1a1a', p: 3, rounded: '2px', border: 'none', color: 'white', mt: 1 })} placeholder="Ex: Sênior Full Stack & IA"/>
              </div>

              <div>
                <label className={css({ color: 'gray.400', fontSize: 'xs' })}>Título Principal</label>
                <input required value={about.title} onChange={e=>setAbout({...about, title:e.target.value})} className={css({ w: 'full', bg: '#1a1a1a', p: 3, rounded: '2px', border: 'none', color: 'white', mt: 1 })}/>
              </div>
              <div>
                <label className={css({ color: 'gray.400', fontSize: 'xs' })}>Subtítulo</label>
                <input required value={about.subtitle} onChange={e=>setAbout({...about, subtitle:e.target.value})} className={css({ w: 'full', bg: '#1a1a1a', p: 3, rounded: '2px', border: 'none', color: 'white', mt: 1 })}/>
              </div>
              <div>
                <label className={css({ color: 'gray.400', fontSize: 'xs' })}>Biografia (Bio)</label>
                <textarea rows={6} required value={about.bio} onChange={e=>setAbout({...about, bio:e.target.value})} className={css({ w: 'full', bg: '#1a1a1a', p: 3, rounded: '2px', border: 'none', color: 'white', mt: 1, resize: 'none' })}/>
              </div>
